    ...
    if run_lsh:
        if write_result:
            with open(result_path + "results.csv", 'w') as out:
                out.write("t,comparisons,pq,pc,f1,comparisons_old,pq_old,pc_old,f1_old\n")
...
def do_lsh(data_list, duplicates, t):
    ...
    correct = np.where(np.array(duplicates, dtype=int) + candidates == 2, 1, 0)
...
def bootstrap(data_list, duplicates):
    ...
    duplicates_sample = np.take(np.take(np.array(duplicates), indices, axis=0), indices, axis=1).tolist()
